...
function cellsFor(n){return [1,3,5,7,0,2,4,6,8].slice(0,n)}
function tongFace(n){let cells=cellsFor(n);return '<div class="tileFace"><div class="tongGrid">'+Array.from({length:9},(_,i)=>cells.includes(i)?'<i class="tongDot"></i>':'<i></i>').join('')+'</div></div>'}
function tiaoFace(n){if(n==1)return '<div class="tileFace"><div class="bird">♣</div></div>';let cells=cellsFor(n);return '<div class="tileFace"><div class="tiaoGrid">'+Array.from({length:9},(_,i)=>cells.includes(i)?'<i class="tiaoStick"></i>':'<i></i>').join('')+'</div></div>'}
function tileInner(x){let id=x.id;if(id<9)return `<div class="tileFace wanFace"><span class="top">${['一','二','三','四','五','六','七','八','九'][id]}</span><span class="bottom">萬</span></div>`;if(id<18)return tongFace(id-8);if(id<27)return tiaoFace(id-17);let names=['東','南','西','北','中','發','白'],red=id==31||id==32;return `<div class="tileFace honorFace ${red?'red':''}">${names[id-27]}</div>`}
function tile(x,cls='',dis=false){if(!x)return'';let id=x.id,k=suit(id);return `<button class="tile ${cls} ${k} ${id==S?.dragonId?'dragonTile':''}" ${dis?'disabled':''} title="${x.name}">${tileInner(x)}</button>`}
