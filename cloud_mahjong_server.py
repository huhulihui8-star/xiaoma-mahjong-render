...
function svgTile(body){return `<svg class="tileSvg" viewBox="0 0 100 140" aria-hidden="true">${body}</svg>`}
function svgText(txt,x,y,size,color='black',family='KaiTi, STKaiti, serif',weight='900'){return `<text x="${x}" y="${y}" text-anchor="middle" dominant-baseline="middle" font-family="${family}" font-size="${size}" font-weight="${weight}" fill="${color}">${txt}</text>`}
function wanFace(n){const nums=['一','二','三','四','伍','六','七','八','九'];return svgTile(svgText(nums[n-1],50,38,42,'#111')+svgText('萬',50,86,42,'#b51216'))}
function dot(cx,cy,r=10,red=false){let mid=red?'#c91f27':'#25345f';return `<circle cx="${cx}" cy="${cy}" r="${r}" fill="white" stroke="#1e2a4e" stroke-width="3"/><circle cx="${cx}" cy="${cy}" r="${r*.58}" fill="none" stroke="${mid}" stroke-width="3"/><circle cx="${cx}" cy="${cy}" r="${r*.24}" fill="${mid}"/>`}
function tongFace(n){if(n==1)return svgTile(dot(50,66,22,true)+dot(50,66,12,false));const pos={2:[[50,42,false],[50,90,false]],3:[[31,36,false],[50,67,true],[69,98,false]],4:[[32,40,false],[68,40,false],[32,94,false],[68,94,false]],5:[[31,37,false],[69,37,false],[50,67,true],[31,97,false],[69,97,false]],6:[[31,34,false],[69,34,false],[31,67,true],[69,67,true],[31,100,false],[69,100,false]],7:[[28,32,false],[50,32,false],[72,32,false],[39,62,false],[61,62,false],[39,94,true],[61,94,true]],8:[[31,28,false],[69,28,false],[31,50,false],[69,50,false],[31,76,false],[69,76,false],[31,100,false],[69,100,false]],9:[[26,28,false],[50,28,false],[74,28,false],[26,58,false],[50,58,true],[74,58,false],[26,90,false],[50,90,true],[74,90,false]]}[n];return svgTile(pos.map(p=>dot(p[0],p[1],8,p[2])).join(''))}
function stick(x,y,red=false){let c=red?'#b51216':'#17713a';return `<rect x="${x-5}" y="${y-17}" width="10" height="34" rx="5" fill="#eef7e8" stroke="${c}" stroke-width="3"/><line x1="${x}" y1="${y-13}" x2="${x}" y2="${y+13}" stroke="${c}" stroke-width="3"/>`}
function birdFace(){let eyes=Array.from({length:9},(_,i)=>{let a=(-120+i*30)*Math.PI/180;return dot(50+31*Math.cos(a),42+24*Math.sin(a),5,i==4)}).join('');return svgTile(eyes+`<ellipse cx="50" cy="72" rx="14" ry="22" fill="#111"/><circle cx="43" cy="65" r="4" fill="white"/><circle cx="57" cy="65" r="4" fill="white"/><path d="M31 101 Q50 80 69 101" fill="none" stroke="#1a7f3a" stroke-width="5"/><path d="M38 108 Q50 88 62 108" fill="none" stroke="#1a7f3a" stroke-width="4"/><line x1="43" y1="96" x2="36" y2="116" stroke="#b51216" stroke-width="3"/><line x1="57" y1="96" x2="64" y2="116" stroke="#b51216" stroke-width="3"/>`)}
function tiaoFace(n){if(n==1)return birdFace();const patterns={2:[[50,42],[50,90]],3:[[34,44],[66,44],[50,92]],4:[[34,42],[66,42],[34,92],[66,92]],5:[[34,38],[66,38],[50,66,true],[34,96],[66,96]],6:[[31,34],[50,34],[69,34],[31,90],[50,90],[69,90]],7:[[31,28,true],[50,28],[69,28],[31,62],[50,62],[69,62],[50,98]],8:[[31,28],[50,28],[69,28],[31,56],[50,56],[69,56],[31,90],[69,90]],9:[[31,28],[50,28,true],[69,28],[31,58],[50,58,true],[69,58],[31,90],[50,90,true],[69,90]]};return svgTile(patterns[n].map(p=>stick(p[0],p[1],p[2])).join(''))}
function honorFace(id){const names=['東','南','西','北','白','發','中'];let idx=id-27,color=(idx==5?'#168331':idx==6?'#b51216':'#111');if(idx==4)return svgTile(`<rect x="27" y="25" width="46" height="82" fill="none" stroke="#28314f" stroke-width="6"/><rect x="34" y="32" width="32" height="68" fill="none" stroke="#28314f" stroke-width="2"/>`);return svgTile(svgText(names[idx],50,68,48,color))}
function tileInner(x){let id=x.id;if(id<9)return wanFace(id+1);if(id<18)return tongFace(id-8);if(id<27)return tiaoFace(id-17);return honorFace(id)}
