...
class Room:
    ...
    def state(self,seat):
        with self.lock:
            ...
            return {'room':self.room_id,'phase':self.phase,'ownerSeat':self.owner,'seat':seat,'name':p.name,'current':self.players[self.current].name,'currentSeat':self.current,'wall':len(self.wall),'dragon':TILE_NAMES[self.dragon] if self.phase=='playing' else '未开局','dragonId':self.dragon,'remaining':rem,'canStart':self.phase=='lobby' and seat==self.owner and self.all_ready(),'canReady':self.phase=='lobby' and p.human,'ready':p.ready,'canAct':self.phase=='playing' and self.current==seat and self.must_discard,'lastDiscard':({'id':self.last_discard,'name':TILE_NAMES[self.last_discard]} if self.last_discard is not None else None),'lastDiscarder':self.last_discarder,'lastDiscarderName':(self.players[self.last_discarder].name if self.last_discarder is not None else ''),'hand':[{'id':t,'name':TILE_NAMES[t]} for t in p.hand],'players':[{'seat':i,'name':q.name,'human':q.human,'ready':q.ready,'owner':i==self.owner,'score':q.score,'handCount':len(q.hand),'discards':[{'id':t,'name':TILE_NAMES[t]} for t in q.discards[-28:]]} for i,q in enumerate(self.players)],'log':self.log[-18:]}
